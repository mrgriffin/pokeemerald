 ...
 *
 * You need to have mgba-rom-test installed. It can be built from source
 * by passing -DBUILD_ROM_TEST=ON to cmake.
 *
 ...
 * STATUS_ICON says that the opponent's HP bar should show a PRZ icon.
 *
 ...
 *           TURN { MOVE(player, MOVE_STUN_SPORE); } // 3.
 *       } SCENE {
 *           NONE_OF { ANIMATION(ANIM_TYPE_MOVE, MOVE_STUN_SPORE, player); } // 4.
 ...
 *
 * NONE_OF inverts the meaning of a SCENE check, so applying it to
 * ANIMATION requires that the Stun Spore animation does not play.
 * MESSAGE checks that the message was shown. The checks in SCENE are
 * ordered, so together this says "The doesn't affect message is shown,
 * and the Stun Spore animation does not play at any time before that".
 * Normally you would only test one or the other, or even better, just
 * NONE_OF { STATUS_ICON(opponent, paralysis: TRUE); } to say that
 * Oddish was not paralyzed without specifying the exact outputs which
 * led to that.
 *
 ...
 * Spaces in pattern match newlines in the message. Often used to check
 * that a battler took its turn, e.g.:
 *     MESSAGE("Wobbuffet used Celebrate!");
 *
 * STATUS_ICON(battler, none: | sleep: | poison: | burn: | freeze: | paralysis:, badPoison:)
 ...
 *
 * ONLY_IF(cond)
 * If cond is true, causes the test to fail if the SCENE command fails.
 * If cond is false, causes the test to fail if the SCENE command
 * succeeds.
...
#define NOT NONE_OF
#define ONLY_IF(cond) for (OpenQueueGroup(__LINE__, (cond) ? QUEUE_GROUP_ONE_OF : QUEUE_GROUP_NONE_OF); gBattleTestRunnerState->data.queueGroupType != QUEUE_GROUP_NONE; CloseQueueGroup(__LINE__))
