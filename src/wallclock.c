...
static void Task_SetClock_HandleInput(u8 taskId)
{
    ...
    else
    {
        ...
        {
            gTasks[taskId].tMoveDir = MOVE_DIR_NONE;

            if (JOY_HELD(DPAD_LEFT))
                gTasks[taskId].tMoveDir = MOVE_DIR_BACKWARD;

            if (JOY_HELD(DPAD_RIGHT))
                gTasks[taskId].tMoveDir = MOVE_DIR_FORWARD;

            if (gTasks[taskId].tMoveDir != MOVE_DIR_NONE)
            {
